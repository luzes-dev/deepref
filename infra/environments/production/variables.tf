variable "expected_account_id" {
  description = "AWS account ID dedicated to production. Plans fail in every other account."
  type        = string
...
variable "availability_zones" {
  description = "Three production availability zones."
  type        = list(string)
...
variable "repository_pull_principal_arns" {
  description = "Cross-account principals allowed to pull production artifacts."
  type        = set(string)
  default     = []
}

variable "promotion_trusted_principal_arns" {
  description = "Principals allowed to assume the production promotion role."
  type        = set(string)
  default     = []
}

variable "promotion_oidc_provider_arn" {
  description = "GitHub Actions IAM OIDC provider ARN used by the production promotion role."
  type        = string
}

variable "promotion_oidc_subjects" {
  description = "Explicit GitHub OIDC subjects allowed to assume the production promotion role."
  type        = set(string)
}

variable "promotion_source_repository_arns" {
  description = "Staging repositories readable by the promotion role."
  type        = set(string)
}
...
variable "database_allocated_storage_gib" {
  description = "Initial production RDS storage."
  type        = number
  default     = 200
}

variable "database_max_allocated_storage_gib" {
  description = "Production RDS storage autoscaling ceiling."
  type        = number
  default     = 1000
}
