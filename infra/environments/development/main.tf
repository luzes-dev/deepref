locals {
  ...
  public_subnet_cidrs = [
    for index in range(3) : cidrsubnet(var.vpc_cidr, 4, index)
  ]
  private_subnet_cidrs = [
    for index in range(3) : cidrsubnet(var.vpc_cidr, 4, index + 3)
  ]
  data_subnet_cidrs = [
    for index in range(3) : cidrsubnet(var.vpc_cidr, 4, index + 6)
  ]
...
resource "terraform_data" "account_guard" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Refusing to manage development from an unexpected AWS account."
    }
...
module "eks" {
  ...
  stateless_min_size               = 1
  stateless_desired_size           = 1
  stateless_max_size               = 4
  tags                             = local.common_tags
...
module "observability" {
  ...
  log_groups = {
    amp         = { name = "/aws/aps/${local.name}", retention_in_days = 7 }
    application = { name = "/deepref/${local.environment}/application", retention_in_days = 7 }
    adot        = { name = "/deepref/${local.environment}/adot", retention_in_days = 7 }
  }
