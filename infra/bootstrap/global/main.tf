...
resource "terraform_data" "account_and_workspace_guard" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Refusing to bootstrap global state from an unexpected AWS account."
    }

    precondition {
      condition     = terraform.workspace == "default"
      error_message = "Bootstrap roots are isolated; OpenTofu workspaces are forbidden."
    }
  }
}
