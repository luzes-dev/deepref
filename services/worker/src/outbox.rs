...
async fn publish_batch(pool: &PgPool, jetstream: &jetstream::Context) -> anyhow::Result<()> {
    let rows = sqlx::query(
        r#"
        UPDATE event_outbox
...
async fn mark_published(pool: &PgPool, id: Uuid) -> anyhow::Result<()> {
    sqlx::query(
        r#"
        UPDATE event_outbox
...
async fn mark_failed(pool: &PgPool, id: Uuid, error: &str) -> anyhow::Result<()> {
    sqlx::query(
        r#"
        UPDATE event_outbox
