	...
	<Card.Content class="flex flex-col gap-4">
		...
		{:else if !reportId}
			<StatePanel
				...
				description="Choose a report above to request a grounded study grouping suggestion."
			/>
		{:else if pending}
		...
		{:else if !proposal || !payload}
			<StatePanel
				state="empty"
				title="No pending grouping suggestion"
				description="Request a suggestion to compare this report with the project’s study groups."
			/>
		{:else}
			...
			<div data-testid="study-grouping-choice">
				<Surface as="section" tone="inset" class="p-4" label="Suggested destination">
					...
					<p class="mt-2 text-xs text-muted-foreground">
						Accepting applies this proposed membership at the recorded revisions. The
						change can be reversed from the study membership controls.
					</p>
			...
			<div>
				<Surface as="section" tone="inset" class="p-4" label="Grouping rationale">
					<p class="text-sm font-medium">Rationale</p>
					<p class="mt-1 text-sm text-muted-foreground">{payload.rationale}</p>
				</Surface>
			</div>
			<div data-testid="study-grouping-provenance">
				<Surface as="section" tone="inset" class="p-4" label="Typed metadata provenance">
