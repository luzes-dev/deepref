<script lang="ts">
	...
	import type { TagsInputProps } from '../../primitives/tags-input/types';
	import TagsInputTag from '../../primitives/tags-input/tags-input-tag.svelte';
	import TagsInputSuggestion from '../../primitives/tags-input/tags-input-suggestion.svelte';
	...
	const showSuggestions = $derived(
		!suggestionsDismissed && inputFocused && filteredSuggestions.length > 0 && tagIndex === undefined
	);
...
<div
	class={cn(
		'relative flex min-h-[36px] w-full flex-wrap place-items-center gap-1 rounded-md border border-input bg-card focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-ring py-0.5 pr-1 pl-1 selection:bg-primary disabled:opacity-50 aria-disabled:cursor-not-allowed',
		className
	)}
...
>
	...
	<input
		...
		aria-activedescendant={showSuggestions && suggestionIndex !== undefined
